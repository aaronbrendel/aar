...
execute "unzip" do
	...
	group "root"
	action :run
	command "unzip -o -d /tmp /tmp/master.zip"
end
...
execute "move AAR" do
	...
	user "root"
	not_if 'ls /var/www/AAR'
end
 
#remote_directory '/var/www/AAR' do
#	source '/tmp/Awesome-Appliance-Repair/AAR'
#end
...
code <<-EOH
root_dbpswd = 'ch3fp4ss'

import MySQLdb
db = MySQLdb.connect(host='localhost', user='root', passwd=root_dbpswd)
sql_script = open('/tmp/Awesome-Appliance-Repair-master/make_AARdb.sql', 'r').read()
